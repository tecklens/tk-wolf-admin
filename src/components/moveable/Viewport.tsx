...
function getId(el: HTMLElement | SVGElement) {
  return el.getAttribute('data-scena-element-id')!;
}

function isScenaFunctionElement(value: any): value is ScenaFunctionJSXElement {
  return isScenaElement(value) && isFunction(value.type)
}
function getScenaAttrs(el: HTMLElement | SVGElement) {
  ...
  for (let i = 0; i < length; ++i) {
    ...
    if (name === 'data-scena-element-id' || name === 'style') {
      continue
    }
    attrs[name] = value
  }
...
function isScenaElement(value: any): value is ScenaJSXElement {
  return isObject(value) && !isScenaFunction(value)
}
function isScenaFunction(value: any): value is ScenaComponent {
...
export default class Viewport extends React.PureComponent<{
  ...
  public render() {
    ...
    return <div className={"viewport-container"} onBlur={this.props.onBlur} style={style}>
      {this.props.children}
      <div className={"viewport"} {...{ ['data-scena-element-id']: "viewport" }} ref={this.viewportRef}>
  ...
  public renderChildren(children: ElementInfo[]): ScenaJSXElement[] {
    return children.map(info => {
      ...
      if (isString(jsx)) {
        props['data-scena-element-id'] = id;
